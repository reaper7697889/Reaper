...
function initializeDatabase() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS data_templates (
        ...
        template_values TEXT NOT NULL, -- Stores JSON: { "column_id_or_name1": "defaultValue1", ... }
        user_id INTEGER, -- Optional: Who created/owns this template
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (target_database_id) REFERENCES note_databases(id) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL,
        UNIQUE (target_database_id, name)
    );
  `);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_data_templates_target_db ON data_templates (target_database_id);`);
  // Consider a trigger for updated_at on data_templates if needed, like other tables.
  // db.exec(`CREATE TRIGGER IF NOT EXISTS trigger_data_templates_updated_at AFTER UPDATE ON data_templates FOR EACH ROW BEGIN UPDATE data_templates SET updated_at = CURRENT_TIMESTAMP WHERE id = OLD.id; END;`);
